...
class Auth
{
    ...
    public function getMenu($uid, $path)
    {
        ...
        $list = cache('sys:cache:menu1:' . $uid);
        if (!$list) {
            $list = Db::name($this->config['auth_rule'])->field('id,pid,name,title,icon')->where($map)->order('pid asc,sort asc,id asc')->select();
        ...
        $data = [
            'menu'  => Tree::unlimitForLayer($list),
            'crumb' => $crumb,
            'pids'  => $parent_ids,
            'self'  => $self
        ];
